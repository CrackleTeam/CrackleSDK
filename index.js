  ...
  if (this.online) {
    ...
    this.librariesList = [
      ...
      },
      {
        ...
        url: "better-flat-design.js",
        version: "v1.0.0",
      },
      {
        name: "Jameson Compatibility",
        description:
          "Patch Snap! to support Jameson-only primitives. Does not include Jameson libraries.",
        url: "jameson-compat.js",
        version: "v1.2",
      },
    ];
    this.buildContents();
...
CrackleImportLibraryMorph.prototype.buildContents = function () {
  ...
  this.addButton(
    () =>
      fetch(this.path + "mods/" + this.selected.url)
        .then((x) => x.text())
        ...
        function makeModMorph(mod) {
          ...
          const deleteButton = new PushButtonMorph(
            ...
            () => {
              ...
              dlg.destroy();
              window.__crackle__.manageLoadedMods(); // reopen with refreshed list
            },
            "Delete",
          );
